...
def diel_populus(model):
    ...
    cobra.io.write_sbml_model(model, os.path.join('models', 'diel_populus_model_fixed.xml'))

def diel_populus_wo_nitrate(model):
    ...
    diel_models_creator(model,
                        ["sucrose_c", "sulfate_c", "nitrate_c", "his_p", "ile_c",
                         ...
                         "arg_c", "cys_c", "gln_c", "gly_c", "pro_c", "tyr_c", "d_alanine_p",
                         "glt_c", "asn_c", "ser_c", "starch_p", "beta_d_fructose_c", "mal_c",
                         "fum_c", "cit_c"], ["EX_light"], biomass_reaction_id="BiomassRxn")

    cobra.io.write_sbml_model(model, os.path.join('models', 'diel_populus_model_fixed_wo_nitrate.xml'))



if __name__ == '__main__':
    populus_model_path = os.path.join('models', 'Populus_iPop7188_fixed.xml')
    populus_model = cobra.io.read_sbml_model(populus_model_path)
    diel_populus(populus_model)
    diel_populus_wo_nitrate(populus_model)